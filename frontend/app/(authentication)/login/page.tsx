...
/* ── Background wrapper — defined OUTSIDE the page component so React never
   remounts it on keystroke re-renders, which would steal input focus.      */
  ...
  return (
    <div className="text-center space-y-3">
      <div className="relative inline-flex items-center justify-center">
        <div style={{ position: 'absolute', inset: -8, borderRadius: 32, background: 'linear-gradient(135deg, rgba(37,99,235,0.4), rgba(79,70,229,0.3))', filter: 'blur(10px)' }} />
        <div className="relative flex items-center justify-center rounded-3xl" style={{ width: 80, height: 80, background: 'linear-gradient(145deg, #1e40af 0%, #2563eb 45%, #4f46e5 100%)', boxShadow: '0 0 0 1px rgba(255,255,255,0.12) inset, 0 8px 32px rgba(37,99,235,0.45)' }}>
          <span style={{ fontSize: 38, lineHeight: 1, filter: 'drop-shadow(0 2px 6px rgba(0,0,0,0.35))' }}>🎓</span>
        </div>
      </div>
      <div>
        <h1 className="text-4xl font-black tracking-tight" style={{ color: 'var(--text-1)', letterSpacing: '-0.03em' }}>IPMS</h1>
        <p className="text-sm font-medium mt-0.5" style={{ color: 'var(--text-3)' }}>Intelligent Project Monitoring System</p>
      </div>
    </div>
  );
}
          ...
          <div className="card p-8 space-y-5">
            ...
            <form onSubmit={handleForgot} className="space-y-4">
              <div>
                ...
                <input type="email" value={resetEmail} onChange={e => setResetEmail(e.target.value)} className="input" placeholder="you@university.edu" autoFocus />
              </div>
      ...
      <LoginBg>
        <div className="card p-8 text-center space-y-5">
          ...
          <div>
            ...
            <p className="text-sm mt-2" style={{ color: 'var(--text-2)' }}>
              If <span className="font-medium" style={{ color: 'var(--text-1)' }}>{resetEmail}</span> is registered, a reset link has been sent. Check your spam folder if you don't see it.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1.5 text-right">
                <button type="button" onClick={() => { setResetEmail(email.trim()); setState('forgot_pw'); }} className="text-xs font-semibold text-blue-500 hover:text-blue-400 transition">
                  Forgot password?
                </button>
              </div>
            </div>
